...
export function SettingsModal({ status, onClose, onSaved }: { status: AgentStatus; onClose: () => void; onSaved: (status: AgentStatus) => void }) {
  ...
  const [provider, setProvider] = useState(initialProvider); const [model, setModel] = useState(status.model ?? initialPreset.defaultModel); const [apiKey, setApiKey] = useState(""); const [baseUrl, setBaseUrl] = useState(initialPreset.baseUrl); const [maxTokens, setMaxTokens] = useState(String(status.max_output_tokens ?? 32768)); const [inputPrice, setInputPrice] = useState(""); const [outputPrice, setOutputPrice] = useState(""); const [pending, setPending] = useState(false); const [error, setError] = useState("");
