...
function JobProgress({ job, bundle }: { job: Job; bundle: ReplayBundle | null }) {
  ...
  const readyText = bundle?.backend.status === "partial"
    ...
    : "已可选择玩家并开始复盘";
  return <div className={`job-progress ${job.status}`}><div className="job-line"><span className="pulse" /><strong>{JOB_LABELS[job.status] ?? job.status}</strong><span>{terminal ? "" : "请保持页面打开"}</span></div>{!terminal && <div className="progress-track"><i /></div>}{job.status === "ready" && bundle && <p>{mapDisplayName(bundle.replay.map_asset_path)} · {formatDuration(bundle.replay.duration_ms)} · {readyText}</p>}{job.status === "unsupported" && <p>已读取录像，但该服务器版本的完整战斗数据暂不能解析。</p>}{job.error_message && job.status === "failed" && <p>{job.error_message}</p>}</div>;
}
