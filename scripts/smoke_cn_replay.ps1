...
param(
    ...
    [string]$OutputDirectory = (Join-Path $PSScriptRoot '..\artifacts\p0\original')
)
...
if (-not (Test-Path -LiteralPath (Join-Path $parser 'src\CliReader\CliReader.csproj'))) {
    throw "CliReader project was not found in $parser. Run scripts/setup_parser.ps1 first."
}
...
if (Test-Path -LiteralPath $output) {
    ...
    if ($null -ne $existing) {
        throw "Refusing to overwrite existing P0 output: $output"
    }
} else {
...
$stdout = Join-Path $output 'original_stdout.log'
$stderr = Join-Path $output 'original_stderr.log'
$commands = Join-Path $output 'commands.log'

"Replay: $replay" | Set-Content -LiteralPath $commands
"Parser: $parser" | Add-Content -LiteralPath $commands
"dotnet run --project src\\CliReader\\CliReader.csproj -- log <replay>" | Add-Content -LiteralPath $commands
"dotnet run --project src\\CliReader\\CliReader.csproj -- export <replay> --output <output>" | Add-Content -LiteralPath $commands
...
} | ConvertTo-Json -Depth 2 | Set-Content -LiteralPath (Join-Path $output 'smoke_result.json')
